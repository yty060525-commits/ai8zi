...
function answerUsefulElements(record: BaziRecord): LocalChatAnswer | null {
  ...
  /* 调候那一节有两种说法，必须都认：气候不偏枯时是「调候参考：…」，偏枯的盘(严冬无火/盛夏无水)
     ...
     于是气候偏枯那批盘问「喜用五行是什么」时调候一栏凭空消失 —— 批断里有、聊天里没有。 */
...
const KEY_PHRASES: Record<string, string[]> = {
  事业: ['事业', '进取', '开拓', '岗位', '管理', '团队', '平台', '创业', '变动', '转岗', '责任', '进气', '乘势', '拓展人脉',
    /* v5 把讲机会的那句从「可承接更重的责任或推进停滞之事」改成按本期十神与取向现推，
       新增的这些词全部取自 add('事业') 实际吐出的措辞（见 .scratch/sample-prose.txt），不是新造的命理词。 */
    '门户', '婚缘', '住处', '长辈', '这条线', '当权', '受阻', '宜守', '可为',
    /* v5 又把爱情句里笼统的「相引」拆成「相合／冲动」（同一宫被两种力量引动，话不该一样），
       流年的感情动向因此归到【事业】节可检索的词下——照旧只收录正文真出现的措辞。 */
    '相合', '冲动'],
