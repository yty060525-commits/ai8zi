...
export function keyMissingHintOf(): string {
  /* 判的是「已勾选」而不是「已开通」：用户 2026-09-25 要求界面上零痕迹（未开通的人不该看见第四路），
     而这句话在未配置态是**常驻渲染**的。若按「已开通」判，知情者只要用地址栏展开过、哪怕没勾选接管，
     详情页也会一直挂着「使用本地系统」—— 那等于把刚藏起来的东西从另一句话里漏出去。
     代价是：开通了但没勾选的人这里仍指向云端凭据，这本来也是对的（此刻确实得配凭据才能点分析）。 */
  if (isLocalSystemEnabled())
    return 'AI 尚未可用：当前已勾选「使用本地系统」，点「AI 分析」由本机规则引擎直接批断，不需要任何通道凭据；若想改用云端大模型，请取消勾选该项（入口在页面右上角「设置」里展开后的那一节），并在任一服务(DeepSeek / Kimi / Qwen3.8-Flash)里填写访问凭据并保存。';
...
/** 详情页是否摆出「本地系统」这块：只看本机有没有开通。与设置页那块的可见性判据同源
    （两边都是 `isLocalSystemUnlocked()`），避免两处各写一份条件而漂开。 */
const readLocalVisible = () => localSystemUnlocked();
/* 本地系统（原「本地离线·第四路」）：不联网、不调大模型、不耗额度，由 localAnalysis 规则引擎
   ...
   本机已开通第四路时才出现（判据见上面的 readLocalVisible）。 */
...
export function PersonDetail({ personId, onBack, refreshKey = 0 }: PersonDetailProps) {
  ...
  /* 详情页这块「本地系统」跟着设置页那块一起收放：本机没开通（或已撤销开通）时就不摆出来。
     漏判的后果是从没开过第四路的设备会在命盘里看见一整块用不上的入口。
     读数走同一个订阅源：在设置页里撤销开通，不用重进详情页这块就该同步消失。 */
