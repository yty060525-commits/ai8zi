 ...
 * 神煞(经典干支起法) — 确定性规则引擎
 *
 * 流派说明：以下均为流传较广、可逐条核验的经典起法（规则版本 classic-v2-local）：
 *  - 以日干取：天乙贵人、文昌贵人、禄神、羊刃(《子平真诠》「惟五阳有之」，阴干不排)
 *
 * ⚠ 「禄神」与「建禄格」是两套禄位，切勿混用(本文件用前者)：
 *   · 禄神(神煞，断吉凶)：甲寅 乙卯 丙巳 丁午 戊巳 己午 庚申 辛酉 壬亥 癸子 —— 阴干走自己的
 *     禄位(乙禄在卯)，与各排盘软件神煞栏一致。
 *   · 建禄(定格局)：按五行临官，阴阳同宫 —— 甲乙寅 / 丙丁戊己巳 / 庚辛申 / 壬癸亥，
 *     故 nonAiCalculator.STEM_LU 里乙禄记作寅。两者互换会把乙生寅月误判成阳刃格。
