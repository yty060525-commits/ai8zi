...
import { isLocalSystemEnabled, isLocalSystemUnlocked, hideLocalSystem, setLocalSystemEnabled, subscribeLocalSystem } from '../../data/localSystem';
...
export function SettingsPage() {
  ...
  /* 本地系统（本机规则引擎）这一路在界面上**不留任何入口**（用户 2026-09-25：「我要看不出来的隐藏起来。
     只有我自己知道怎么展开，不要让任何人看得出来有展开方式」）。展开方式是地址栏 `?local=<解锁码>`，
     见 localSystem.ts 的 unlockFromLocation；这里只负责：本机已开通时把那一节摆出来、并让它可关。
     可见性判据与详情页同源（isLocalSystemUnlocked），别在这儿另加第二套条件。 */
  const readUnlocked = () => isLocalSystemUnlocked();
  const localUnlocked = useSyncExternalStore(subscribeLocalSystem, readUnlocked, readUnlocked);
  const localOn = isLocalSystemEnabled();
  const [localNote, setLocalNote] = useState<string>();
  const flashLocalNote = (text: string) => { setLocalNote(text); setTimeout(() => setLocalNote(undefined), 4000); };
  /** 「关起来」＝收回这一节，并且**一定**先把生成方式交还给云端（「只要关闭了就默认走 qwen」）。
   *  hideLocalSystem() 内部先关灯再清两个标记；这里不重算判据，靠订阅自己回读实现。 */
  function doHideLocal() {
    hideLocalSystem();
    flashLocalNote('已关起来：本地系统不再接管，「AI 分析」回到云端通道');
  }
  ...
  return <main className="settings-page">
    ...
    {/* 「本地系统」是第四条生成方式：不走云端、不用凭据。**未开通时这一整块连一个字都不出现** ——
        没有提示句、没有输入框、没有按钮，别人看设置页看不出这里还能开什么。
        机主用地址栏 `?local=<解锁码>` 展开（写法与原因见 localSystem.ts）。 */}
    {localUnlocked && <section className="local-system" aria-label="本地系统"><h2>本地系统（本机规则引擎）<span className="channel-status ok">已开通</span>{localNote && <span className="channel-notice" role="status">{localNote}</span>}</h2>
      ...
      <div className="local-unlock-row">
        ...
        <button className="text-button" type="button" onClick={doHideLocal}>关起来（改用云端通道）</button>
      </div>
    </section>}
