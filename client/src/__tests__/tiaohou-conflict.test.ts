 ...
 *   ⚠ 这里原先记的 `TOTAL=120 CONFLICT=27 / 春:8/30 夏:4/30 秋:7/30 冬:8/30` 是**错的**：那份枚举
 *   把春秋也算进分子，可春秋依季节通义本就「调候非急」、压根没有需要权衡的急务。
 *   重算口径（只数夏/冬 × 10 日主 = 60 组，且直接读 nonAiCalculator 的两张表本体）：
 *     身强档 CONFLICT=30（夏 18/冬 12）、身弱档 CONFLICT=45（冬 24/夏 21）；
 *     其中「所需之气本身就是喜用、只有《穷通》另取的辅佐干撞忌神」= 6 组(身强)/9 组(身弱)。
 *   旧写法对这些盘一律端「调候非急」或一句泛泛的「辅助判据」，等于把分歧藏起来。
...
describe('调候与扶抑冲突时的本命批断', () => {
  ...
  it('聊天同口径：偏枯盘的调候分歧句必须也能被「喜用五行是什么」取到', () => {
    /* 缺陷现场（本轮读码发现，非假想）：localChat 的 answerUsefulElements 原先只认
       ...
       聊天答出来那一栏凭空少一条（凡气候偏枯之盘皆中招）。修法是让它同时认「此系两源出入」。 */
    const birth = BIRTH;
